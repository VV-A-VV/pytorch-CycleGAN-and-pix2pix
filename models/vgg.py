...
class ContextualLoss(nn.Module):
    def __init__(self):
        ...
        # get specific layer
        self.conv3_2 = dict(self.model.named_children())['features'][12]
        self.conv4_2 = dict(self.model.named_children())['features'][21]
        # get feature
        self.custom_model1 = CustomVGG(self.model, 12+1)  
        self.custom_model2 = CustomVGG(self.model, 21+1)
    def cx(self,image1,image2):
        # compute
        with torch.no_grad():
            feature_map1_image1 = self.custom_model1(image1)
            feature_map1_image2 = self.custom_model1(image2)
            feature_map2_image1 = self.custom_model2(image1)
            feature_map2_image2 = self.custom_model2(image2)

        # comput CX loss
        cx_loss1 = CX_loss(feature_map1_image1, feature_map1_image2)
        cx_loss2 = CX_loss(feature_map2_image1, feature_map2_image2)


        final_loss = (cx_loss1 + cx_loss2) / 2
...
class CustomVGG(nn.Module):
    def __init__(self, original_model, end_layer):
        super(CustomVGG, self).__init__()
        self.features = nn.Sequential(*list(original_model.features.children())[:end_layer])
